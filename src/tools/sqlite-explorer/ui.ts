...
export function renderTableList(tables: string[], onSelect: (table: string) => void) {
  ...
  UI.tableList.innerHTML = tables
    .map(
      (name) =>
        html`<li>
          <a href="#" data-table="${name}"
            ><i data-lucide="table" class="w-4 h-4 mr-1 md:mr-2"></i>
...
export function renderSchemaTable(schemaRows: any[]) {
  UI.schemaTableBody.innerHTML = schemaRows
    .map((row) => {
...
export function renderDataTable(
  ...
  if (columns.length === 0) {
    ...
    UI.dataTableBody.innerHTML =
      '<tr><td class="text-center p-4 text-base-content/50">No data found in this table.</td></tr>';
  } else {
    UI.dataTableHead.innerHTML = html`<tr>
      ${columns.map((c) => `<th>${c}</th>`).join('')}
    </tr>`;
    UI.dataTableBody.innerHTML = rows
      .map(
        (row) =>
          html`<tr>
            ${row
...
export function renderCustomQueryResult(columns: string[], rows: any[][]) {
  ...
  UI.queryResultsCount.textContent =
    rows.length > 0 ? `${rows.length} rows returned` : 'No rows returned';

  if (columns.length === 0) {
    UI.queryResultsHead.innerHTML = '';
    UI.queryResultsBody.innerHTML =
      '<tr><td class="text-center p-4 text-base-content/50">Query executed successfully. No data returned.</td></tr>';
  } else {
    UI.queryResultsHead.innerHTML = html`<tr>
      ${columns.map((c) => `<th>${c}</th>`).join('')}
    </tr>`;
    UI.queryResultsBody.innerHTML = rows
      .map(
        (row) =>
          html`<tr>
            ${row
