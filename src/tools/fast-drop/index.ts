...
export default function init() {
  ...
  const fetchFiles = async () => {
    ...
    if (data.success) {
      ...
      showErrorState(data.error || 'Unknown error');
    }
    listLoading.classList.add('hidden');
  };

  const showErrorState = (error: string) => {
    console.error('[FastDrop] error:', error);
    ...
    listEmpty.innerHTML = `
      <div class="text-center py-8">
        ...
        <p class="text-xs opacity-50 mt-1">${error}</p>
      </div>
    `;
  ...
  const renderFiles = (drops: Drop[]) => {
    ...
    drops.forEach((drop) => {
      const card = createFileCard(drop);
  ...
  const createFileCard = (drop: Drop) => {
    ...
    const sizeText = utils.formatSize(drop.size);
    const icon = utils.getFileIcon(drop.type);
  ...
  const deleteFile = async (id: string) => {
    ...
    const data = await api.deleteDrop(id);
    if (data.success) {
      fetchFiles();
    } else {
  ...
  const keepFile = async (id: string) => {
    const data = await api.keepDrop(id);
    if (data.success) {
      fetchFiles();
    } else {
  ...
  const previewFile = async (drop: Drop) => {
    ...
    try {
      const blob = await api.fetchDropBlob(drop.id);
      objectUrl = URL.createObjectURL(blob);

      configurePreviewCopy(drop, blob);
      renderPreviewContent(drop, blob, objectUrl);
  ...
  const configurePreviewCopy = (drop: Drop, blob: Blob) => {
    const canCopy =
  ...
  const renderPreviewContent = async (drop: Drop, blob: Blob, objectUrl: string) => {
    if (drop.type.startsWith('image/')) {
    ...
    try {
      ...
      for (const item of items) {
        for (const type of item.types) {
          ...
          const filename = generatePastedFilename(type);
  ...
  const generatePastedFilename = (type: string): string => {
    let extension = '';
    const now = Date.now();
    if (type.startsWith('image/')) {
      ...
      return `pasted-text-${now}.txt`;
    } else if (type === 'text/html') {
      return `pasted-content-${now}.html`;
    } else {
      const extMatch = type.match(/\/([a-z0-9]+)$/);
      extension = extMatch ? extMatch[1] : 'bin';
      return `pasted-file-${now}.${extension}`;
    }
  };

  const handleTextPasteFallback = async () => {
    try {
