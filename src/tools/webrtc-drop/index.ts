...
export default async function init() {
    ...
    bc.onmessage = (ev) => {
        ...
        } else if (msg.type === 'offer' && !manager) {
        ...
        } else if (msg.type === 'answer' && manager && !manager.isStable) {
             manager.processSDP(msg.sdp);
        }
    };
    ...
    function showConnected() {
        if (!manager || manager.iceConnectionState !== 'connected' || !manager.hasRemoteDescription || !remotePeerNameEl.textContent) {
            return;
        }
    ...
    function showQR(text: string) {
        ...
        QRCode.toCanvas(qrCanvasOutput, text, { width: 600, margin: 1, color: { dark: '#000000', light: '#ffffff' } }, (err) => {
    ...
    function handleData(data: any) {
        if (typeof data === 'string') {
            ...
            } catch (e) { console.warn('Malformed string message', e); }
        } else {
    ...
    function processChunk(size: number) {
        ...
        if (receivedSize >= incomingMeta.size) {
            ...
            addToHistory({ name: incomingMeta.name, size: receivedSize, type: 'received', blob: receivedSize < 50 * 1024 * 1024 ? blob : null });
            incomingChunks = [];
            receivedSize = 0;
            incomingMeta = null;
    ...
    async function sendFile(file: File) {
        ...
        manager.send(JSON.stringify({ type: 'metadata', name: file.name, size: file.size, mime: file.type }));
    ...
    function addToHistory(item: { name: string, size: number, type: 'sent' | 'received', blob?: Blob | null }) {
